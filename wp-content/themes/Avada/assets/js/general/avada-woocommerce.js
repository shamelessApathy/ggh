...
function variationsChange( imageThumbs ) {
	var sources = window.sources,
	    variation,
	    variationImage,
	    variationSelects = getVariationsValues();

	// Get the variation params.
	jQuery( '.variations_form' ).on( 'found_variation', function( event, variationParam ) {
		variation = variationParam;

	});

	variationImage = ( jQuery( '.flex-control-nav' ).find( 'img' ).length ) ? jQuery( '.flex-control-nav' ).find( 'li:eq(0) img' ).attr( 'src' ) : undefined;

	if ( 'undefined' === typeof variationImage && variationSelects.variations !== variationSelects.chosen ) {
		jQuery( '.variations_form' ).trigger( 'update_variation_values' );
		jQuery( '.variations_form' ).trigger( 'reset_data' );
	} else {

		if ( 'undefined' !== typeof variationImage && -1 !== jQuery.inArray( variationImage, sources ) ) {
			variationImage = sources[0];
		} else if ( 'undefined' !== typeof variation ) {
			variationImage = variation.image.src;
		}
		...
		imageThumbs.each( function() {
			...
			if ( ! jQuery( this ).hasClass( 'fusion-main-image-thumb' ) ) {
				...
				$productImg      = jQuery( '.flex-viewport' ).find( '.fusion-main-image img' ),
				lightboxTrigger = jQuery( '.flex-viewport' ).find( '.fusion-main-image .avada-product-gallery-lightbox-trigger' );

				jQuery( this ).attr( 'src', variationImage );

				if ( variationImage === sources[0] ) {
				...
				} else if ( 'undefined' !== typeof variation && variation.image && variation.image.src && variation.image.src.length > 1 ) {
