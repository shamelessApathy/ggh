...
<div class="wrap about-wrap avada-wrap">
	...
	<div class="avada-system-status">
		...
		<table class="widefat" cellspacing="0">
			...
			<tbody>
				...
				<tr>
					...
					?>
					<td><?php echo wp_kses_post( $previous_version ); ?></td>
				</tr>
				<?php
					...
					<td>
						...
						<?php else : ?>
							...
							<?php if ( $memory < 256000000 ) : ?>
								<br />
								<mark class="error">
									<?php printf( esc_attr__( 'Your current memory limit is sufficient, but if you need to import classic demo content, the required memory limit is %s', 'Avada' ), '<strong>256MB.</strong>' ); ?>
								</mark>
							<?php endif; ?>
						<?php endif; ?>
					</td>
					...
					<tr>
						...
						<td>
							...
							if ( 180 > $time_limit && 0 != $time_limit ) {
								echo wp_kses_post( '<mark class="error">' . sprintf( __( '%1$s - We recommend setting max execution time to at least 180. <br /> To import classic demo content, <strong>300</strong> seconds of max execution time is required.<br />See: <a href="%2$s" target="_blank" rel="noopener noreferrer">Increasing max execution to PHP</a>', 'Avada' ), $time_limit, 'http://codex.wordpress.org/Common_WordPress_Errors#Maximum_execution_time_exceeded' ) . '</mark>' );
							} else {
								echo '<mark class="yes">' . esc_attr( $time_limit ) . '</mark>';
								if ( 300 > $time_limit && 0 != $time_limit ) {
									echo wp_kses_post( '<br /><mark class="error">' . __( 'Current time limit is sufficient, but if you need import classic demo content, the required time is <strong>300</strong>.', 'Avada' ) . '</mark>' );
								}
							}
							?>
						</td>
					...
					<tr>
						...
						<td>
							...
							if ( $max_input_vars < $required_input_vars ) {
								echo wp_kses_post( '<mark class="error">' . sprintf( __( '%1$s - Recommended Value: %2$s.<br />Max input vars limitation will truncate POST data such as menus. See: <a href="%3$s" target="_blank" rel="noopener noreferrer">Increasing max input vars limit.</a>', 'Avada' ), $max_input_vars, '<strong>' . $required_input_vars . '</strong>', 'http://sevenspark.com/docs/ubermenu-3/faqs/menu-item-limit' ) . '</mark>' );
							} else {
		...
		<table class="widefat" cellspacing="0" id="status">
			<thead>
				<tr>
					<th colspan="3" data-export-label="Active Plugins (<?php echo count( (array) get_option( 'active_plugins' ) ); ?>)"><?php esc_attr_e( 'Active Plugins', 'Avada' ); ?> (<?php echo count( (array) get_option( 'active_plugins' ) ); ?>)</th>
				</tr>
			</thead>
			<tbody>
				<?php
				...
				foreach ( $active_plugins as $plugin ) {
					...
					if ( ! empty( $plugin_data['Name'] ) ) {
						...
						<tr>
							...
							<td class="help">&nbsp;</td>
							<td><?php echo wp_kses_post( sprintf( esc_attr__( 'by %s', 'Avada' ), '<a href="' . esc_url( $plugin_data['AuthorURI'] ) . '" target="_blank">' . esc_html( $plugin_data['AuthorName'] ) . '</a>' ) . ' &ndash; ' . esc_html( $plugin_data['Version'] ) . $version_string . $network_string ); ?></td>
						</tr>
						<?php
					}
				}
