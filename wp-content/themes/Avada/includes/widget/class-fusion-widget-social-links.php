		...
		if ( is_array( $theme_options_array ) && array_key_exists( 'icon', $theme_options_array ) ) {
			...
			if ( isset( $custom_icon_indices ) && is_array( $custom_icon_indices ) ) {
				foreach ( $custom_icon_indices as $name => $index ) {
					...
					$i++;
					$custom_icons[ $icon_key ] = array( 'label' => 'Custom ' . $i . ' (' . $theme_options_array['custom_title'][ $index ] . ')',  'color' => '' );
				}
			}
		}
				...
				<?php foreach ( $social_networks_ordered as $name => $value ) : ?>
					...
					if ( 'brand' == $instance['color_type'] ) {
						...
						if ( is_string( $value ) ) {
							...
							$social_icon_boxed_colors['mail'] = array( 'label' => esc_html__( 'Email Address', 'Avada' ), 'color' => '#000000' );
					...
					<?php else : ?>
						...
						<?php endif; ?>
						<a class="fusion-social-network-icon fusion-tooltip" target="<?php echo esc_attr( $instance['linktarget'] ); ?>" href="<?php echo esc_url_raw( $value['network_link'] ); ?>" rel="<?php esc_attr( $nofollow ); ?>" <?php echo wp_kses_post( $tooltip_params ); ?> title="" style="<?php echo esc_attr( $style ); ?>"><img src="<?php echo esc_url_raw( $value['network_icon'] ); ?>" height="<?php echo esc_attr( $value['network_icon_height'] ); ?>" width="<?php echo esc_attr( $value['network_icon_width'] ); ?>" alt="<?php echo esc_attr( $value['network_name'] ); ?>" /></a>
					<?php endif; ?>
